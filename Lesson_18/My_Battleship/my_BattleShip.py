"""
    ...
    Шляхом надання клітинкам поля коефіціента значимості (ваги) в грі
    реалізовано елементи штучного інтелекту (компютер розрізняє клітинки,
    де з великою вірогідністю можуть знаходитись кораблі гравця).
    ...
    потоплений корабель    = '🟥'
    підбитий корабель     = '🟥'
    невлучний постріл або сіра зона
    (частина поля, що не може бути використана) = '🟩'
"""
...
class Board:
    ...
    def __init__(self, size):
        ...
        """
        self.size = size
        self.map_user = [[Cell.empty_cell for _ in range(size)]
                         for _ in range(size)]
        self.map_comp = [[Cell.empty_cell for _ in range(size)]
                         for _ in range(size)]
        self.weight = [[1 for _ in range(size)] for _ in range(size)]

    def get_board_part(self, element):
        """
        Метод з обовязковим праметром 'element', який дає змогу повернути
        значення інших обєктів до однієї з 3 частин поля ('частина гри гравця',
        частина гри компютера', 'значимість (вага) клітинки поля')
    ...
    def create_board(self, element):
        """
        Метод, який свторює та виводить в термінал поля гравця
        та комп'ютера (окремо щодо поля та ваги клітинок)
        """
    ...
    def check_position_ship(self, ship, element):
        """
        Метод здійснює перевірку можливості розміщення корабля в
        конкретному місці (використвоується при розміщенні кораблів та
        вирахування значимості (ваги) клітинок).
        ...
        for p_x in range(x, x + height):
            # цикл для перевірки чи не відповідають координати сірій зоні
            # або вже відстріляним координатам.
        ...
        for p_x in range(x - 1, x + height + 1):
            ...
            for p_y in range(y - 1, y + width + 1):
                if p_x < 0 or p_x >= len(field) or p_y < 0 or p_y >= len(field):
                    continue
                if str(field[p_x][p_y]) in (Cell.ship_cell,
                                            Cell.destroyed_ship):
                    return False
    ...
    def add_ship_to_board(self, ship, element):
        """
        Метод додавання корабля на поле (формуєм коордианати шляхом
        перeбирання його висоти та ширини через цикл)
        """
    ...
    def get_max_weight_cells(self):
        ...
        Метод повертає список координат із самим більшим шансом попадання.
         Перебираємо через цикли всі клітини та заносимо їх до словника weights
         = {} з ключем, який є значенням в клітинці. Через append додаємо
        (запамятовуємо) максимальне значення.
    ...
    def recalculate_weight_map(self, available_ships):
        """
        Метод перераховує значення (вагу) клітинок, оскільки після кожного ходу
        їх вага змінюється.
        Всім клітинкам зпочатку надається значення 1.
        Пробігаємо по всьому полю.
        Якщо знаходимо поранений корабель - ставимо клітинам вище/нижче/з
        боків коефіцієнти помножені на 50
        (є великий шанс, що там є продовження корабля).
        За діагоналями від пораненої клітини нічого не може бути –
        туди вписуємо нулі. Якщо на карті знищений корабель або клітка з
        промахом - ставимо туди коефіцієнт 0.
        Далі переходимо до наступної клітини. Якщо в клітині є можливість
        поміститися кораблю додаємо їй коеф 1.
        """
        ...
        for ship_size in available_ships:
            ...
            for x in range(self.size):
                for y in range(self.size):
                    if self.map_comp[x][y] in (Cell.destroyed_ship,
                                               Cell.damaged_ship,
                                               Cell.miss_cell) \
...
class Game:
    ...
    def status_check(self):
        """
        Метод переключає статус гри. Переключаємо з "prepare" на "in game"
        якщо до гри доданго 2 гравця.
        Далі стартуємо гру
        ...
        if self.status == 'prepare' and len(self.players) >= 2:
            ...
            self.start_game()
            return True
        # переключаємо в статус game over
        # якщо у наступного гравця заишилось 0 кораблів
    ...
    def ships_setup(self, player):
        ...
        for ship_size in Game.ships_rules:
            # задаємо кількість спроб виставлення кораблів (40)
            # Щоб не потрапити в бескінечний цикл
            retry_count = 40
            ...
            while True:
                ...
                if player.auto_ship_setup is not True:
                    ...
                    player.message.append('Куди поставити {} корабель: '.
                                          format(ship_size))
                ...
                # якщо гравець ввів некоректні координати (зачення = 0),
                # то просимо повторно ввести
                if x + y + r == 0:
                    continue

                ship.set_position(x, y, r)

                # якщо корабель міститься на заданій позиції - додаємо гравцю
                # на поле корабель також додаємо корабель до списку кораблів
                # гравця і переходимо до наступного корабля для розміщення
...
class Player:
    ...
    def get_input(self, input_type):
        """
        Метод прийняття ходу гравця. Або розстанока кораблів
        (input_type == "ship_setup") aбо постріл (input_type == "shot")
        """

        if input_type == "ship_setup":

            if self.is_ai or self.auto_ship_setup:
                user_input = str(choice(Game.abc)) + str(randrange(
                    0, self.field.size)) + choice(["H", "V"])
            else:
            ...
            if x not in Game.abc or not y.isdigit() or int(y) not in range(
                    1, Game.field_size + 1) or r not in ("H", "V"):
        ...
        if input_type == "shot":
            ...
            if self.is_ai:
                ...
                if self.skill == 0:
                    x, y = randrange(0, self.field.size),\
                           randrange(0, self.field.size)
            else:
                user_input = input().upper().replace(" ", "")
                x, y = user_input[0].upper(), user_input[1:]
                if x not in Game.abc or not y.isdigit() or int(y) not in\
                        range(1, Game.field_size + 1):
                    self.message.append('Помилка при введенні координат!')
                    return 500, 0
                x = Game.abc.index(x)
                y = int(y) - 1
...
if __name__ == '__main__':
    ...
    choice_ = input("Оберіть тип розстановки кораблів ((1)автоматично),"
                    " (2) в ручному режимі: ")
    assert choice_ != "a" or "b", "помилка"
    if choice_ == "1":
        players = []
        players.append(Player(name='Гравець', is_ai=False, auto_ship=True,
                              skill=1))
        players.append(Player(name="Комп'ютер", is_ai=True, auto_ship=True,
                              skill=1))
    elif choice_ == "2":
        players = []
        players.append(Player(name='Гравець', is_ai=False, auto_ship=False,
                              skill=1))
        players.append(Player(name="Комп'ютер", is_ai=True, auto_ship=True,
                              skill=1))
    ...
    while True:
        game.status_check()  # кожен початок ходу перевіряємо статус і далі
        # вже діємо виходячи зі статусу гри
        ...
        if game.status == 'in game':
            ...
            Game.clear_screen()  # очищаємо екран та додаємо повідомлення для
            # поточного гравця та малюємо гру
            game.current_player.message.append("Введіть координати для пострілу"
                                               " (приклад: 'a2'): ")
            game.draw()
            ...
            if shot_result == 'miss':\
                ...
                game.next_player.message.append(
                    '{} промахнувся! '.format(game.current_player.name))
                game.next_player.\
                    message.append('Хід: -> {}!'.format(game.next_player.name))
                game.switch_players()
                continue
                ...
                game.next_player.message.append('Корабель був повністю '
                                                'знищений!')
                continue
