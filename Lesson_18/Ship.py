class Ship():
    ...
    status_map = [] свойство:массив с координатами точек корабля, который формируется конструктором
    around_map = [] :координаты точек вокруг корабля
    свойство:статус гибели корабля
    prefix = ""(указывается при создании объекта):префикс тега (для своих кораблей будет, например, "my", для чужих "nmy"
